...
// Coordinates come from the client's "Auto-fill details" button (see
// lib/mapsLink.js) — optional here because the insert routes fall back to
// pincode geocoding when they're missing.
function optionalCoordinate(value, max) {
  if (value === undefined || value === null || value === "") return null;
  const n = Number(value);
  return Number.isFinite(n) && Math.abs(n) <= max ? n : null;
}

function nonNegativeOrDefault(value, fallback = 0) {
  const n = positiveNumber(value);
  return n === null ? fallback : n;
}

// Admin-only field (see components/admin/AdminPackagesField.jsx) — unlike
// the public submit route, which re-derives promotions/total server-side
// from the DB via resolvePromotionSelections (lib/promotionPackages.js) to
// avoid trusting client-sent prices, this route is admin-session-gated, so
// the admin's own edited prices/discount are trusted directly. Still
// type/range-validated so a malformed payload can't corrupt the row —
// malformed individual entries are dropped rather than failing the whole
// submission, same spirit as the poster-brief fields above.
function optionalPromotions(value) {
  if (!Array.isArray(value)) return [];
  return value
    .map((entry) => {
      const name = trimmed(entry?.name, 120);
      const price = positiveNumber(entry?.price);
      const subtotal = positiveNumber(entry?.subtotal);
      if (!name || price === null || subtotal === null) return null;
      const quantity = Number.isInteger(Number(entry?.quantity)) && Number(entry.quantity) > 0
        ? Number(entry.quantity)
        : 1;
      return {
        package_id: trimmed(entry?.package_id, 100) || null,
        name,
        price,
        price_unit: trimmed(entry?.price_unit, 40),
        quantity,
        subtotal,
      };
    })
 ...
 */
export function parseTournamentInput(body, { posterOptional = false } = {}) {
  ...
  const prize_pool_is_trophy = Boolean(body.prize_pool_is_trophy);
  // "Trophy alone" tournaments have no cash winner prize — prize_pool stays
  // 0 and the amount input is neither shown nor required in that case.
  const prize_pool = prize_pool_is_trophy ? 0 : positiveNumber(body.prize_pool);
  ...
  // Actual teams that took part — usually unknown at submission time and
  // filled in later (see supabase/schema.sql), so not a required field.
  const total_teams = optionalNonNegativeInt(body.total_teams);
  ...
  const promotions = optionalPromotions(body.promotions);
  const promotion_total = nonNegativeOrDefault(body.promotion_total);
  ...
  if (
    ...
    !organizer_phone ||
    !instagram_id ||
    (!image_url && !posterOptional) ||
    entry_fee_amount === null ||
    prize_pool === null ||
    slots_left === undefined ||
    max_teams === undefined ||
    total_teams === undefined
  ) {
