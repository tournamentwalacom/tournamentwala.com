...
/**
 * Category used for the auto-inserted per-order Razorpay charge rows (see
 * /api/tournaments/submit). Kept as a real "expenses" row so each order's
 * charge is individually visible/editable, but excluded from "manual
 * expenses" sums wherever they're computed (admin Expenses + Analytics)
 * since the summary cards still surface the live 2.5%-of-income figure
 * separately — summing both would double-count it.
 */
export const RAZORPAY_CATEGORY = "Razorpay";
