...
function excludeExpired(query) {
  ...
  return query.or(
    `and(end_date.is.null,start_date.gte.${today}),end_date.gte.${today}`
  );
}
...
/** Compact Indian-style number: 45000 -> "45,000", 850000 -> "8.5 L",
 * 42000000 -> "4.2 Cr". Used for both the rupee stats and the teams count. */
...
export function formatPrizesPaidOut(amount) {
  return `₹${formatIndianCompact(amount)}`;
}

export function formatTeamsCount(n) {
  return formatIndianCompact(n);
}

export function formatTournamentsHosted(n) {
...
export function formatPrize(tournament) {
  if (tournament.prize_pool_is_trophy) return "Trophy";
